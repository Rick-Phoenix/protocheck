...
impl PartialOrd for Money {
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    ...
    // i128 is used because self.units (i64) * 1_000_000_000 might exceed i64 max.
...
impl Money {
  ...
  /// DISCLAIMER: all of the methods implemented for Money are just implemented for convenience, and they are provided as is, without warranties of any kind. By using this module, the user is relieving the authors of this library from any responsibility for any damage that may be caused by its usage.
  pub fn to_rounded_imprecise_f64(&self, decimal_places: u32) -> f64 {
    let full_amount = self.as_imprecise_decimal_f64(); // Get the full precision amount first

    // Calculate the factor for rounding
    let factor = 10.0f64.powi(decimal_places as i32);

    // Round to the specified decimal places
    (full_amount * factor).round() / factor
  }
  ...
  pub fn imprecise_from_decimal_f64(
    ...
    let nanos = (amount.fract().abs() * 1_000_000_000.0).round() as i32;
  ...
  pub fn try_mul_i64(&self, rhs: i64) -> Result<Self, MoneyError> {
    ...
    Money::new(self.currency_code.clone(), mul_units, mul_nanos_i64 as i32)
  }
  ...
  pub fn try_mul_f64(&self, rhs: f64) -> Result<Self, MoneyError> {
    ...
    let decimal_amount = self.as_imprecise_decimal_f64(); // Use the full-precision getter here
  ...
  pub fn try_div_i64(&self, rhs: i64) -> Result<Self, MoneyError> {
    ...
    // Convert the entire Money amount to total nanos (i128) for high-precision division
    ...
    let new_units = (result_total_nanos / NANO_FACTOR as i128) as i64;
