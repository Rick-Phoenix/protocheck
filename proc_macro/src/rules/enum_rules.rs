...
pub fn get_enum_rules(
  ...
  if let Some((in_list, in_list_str)) = in_list {
    ...
    let validator_expression_tokens = quote! {
      protocheck::validators::containing::enum_in_hashset_list(&#field_context_ident, #value_ident, &#in_list_ident, #error_message)
    };
  ...
  if let Some((not_in_list, not_in_list_str)) = not_in_list {
    ...
    let validator_expression_tokens = quote! {
      protocheck::validators::containing::enum_not_in_hashset_list(&#field_context_ident, #value_ident, &#not_in_list_ident, #error_message)
    };
