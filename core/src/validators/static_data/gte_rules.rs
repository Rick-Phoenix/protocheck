...
pub(crate) fn get_gte_rule_path(
  kind: FieldType,
) -> Option<(&'static str, &'static [FieldPathElement])> {
  match kind {
    FieldType::Float => Some(("float", &FLOAT_GTE_VIOLATION)),
    FieldType::Double => Some(("double", &DOUBLE_GTE_VIOLATION)),
    FieldType::Int32 => Some(("int32", &INT32_GTE_VIOLATION)),
    FieldType::Int64 => Some(("int64", &INT64_GTE_VIOLATION)),
    FieldType::Uint32 => Some(("uint32", &UINT32_GTE_VIOLATION)),
    FieldType::Uint64 => Some(("uint64", &UINT64_GTE_VIOLATION)),
    FieldType::Sint32 => Some(("sint32", &SINT32_GTE_VIOLATION)),
    FieldType::Sint64 => Some(("sint64", &SINT64_GTE_VIOLATION)),
    FieldType::Fixed32 => Some(("fixed32", &FIXED32_GTE_VIOLATION)),
    FieldType::Fixed64 => Some(("fixed64", &FIXED64_GTE_VIOLATION)),
    FieldType::Sfixed32 => Some(("sfixed32", &SFIXED32_GTE_VIOLATION)),
    FieldType::Sfixed64 => Some(("sfixed64", &SFIXED64_GTE_VIOLATION)),
    FieldType::Duration => Some(("duration", &DURATION_GTE_VIOLATION)),
    FieldType::Timestamp => Some(("timestamp", &TIMESTAMP_GTE_VIOLATION)),
    _ => None,
  }
}
