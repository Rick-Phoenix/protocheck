...
use crate::buf::validate::MapRules;
use crate::rules::get_field_rules;

pub fn get_map_rules(map_rules: &MapRules) -> Result<Vec<CelRule>, Box<dyn std::error::Error>> {
  ...
  if map_rules.keys.is_some() {
    ...
    let keys_rules: Vec<CelRule> = get_field_rules(&keys_rules_descriptor)?
      .into_iter()
      .map(|mut rule| {
  ...
  if map_rules.values.is_some() {
    ...
    let values_rules: Vec<CelRule> = get_field_rules(&values_rules_descriptor)?
      .into_iter()
      .map(|mut rule| {
